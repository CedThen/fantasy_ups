...
func _process(delta: float) -> void:
	if not map_camera.zoom.is_equal_approx(target_zoom):
		map_camera.zoom = map_camera.zoom.lerp(target_zoom, delta * zoom_speed)
	else:
		map_camera.zoom = target_zoom
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if is_dragging and event is InputEventMouseMotion:
		...
		global_position -= delta * map_camera.zoom.x

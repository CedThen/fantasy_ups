...
@export var location_size: LocationSize = LocationSize.small

var isMouseHovering := false
var LocationSizeDict: Dictionary[LocationSize, int] = {
	LocationSize.small: 15,
	LocationSize.medium: 25,
	LocationSize.large: 45
}
...
func render_on_map():
	...
	if mouse_collision_shape:
		...
		var new_radius = float(LocationSizeDict[location_size])
...
func _on_highlight_area_input_event(viewport: Node, event: InputEvent, shape_idx: int) -> void:
	...
	and event.button_index == MOUSE_BUTTON_LEFT:
		print("Area clicked " + location_name)

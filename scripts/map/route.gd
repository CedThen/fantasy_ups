...
func render_route():
	...
	if origin and destination:
		...
		var perpendicular = Vector2(-direction.y, direction.x)  # 90 degrees CCW
		info_panel.position = mid_point + perpendicular * OFFSET_DISTANCE
		weight_label.text = "Weight: " + str(weight)
